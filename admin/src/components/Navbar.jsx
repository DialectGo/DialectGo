...
const Navbar = ({ title }) => {
  ...
  const fetchNotifications = async () => {
    ...
    try {
      ...
      if (!token) return; // no token = not logged in, skip silently
    ...
    >
      ...
      <div className="navbar-right" style={{ display: 'flex', alignItems: 'center', gap: '24px' }}>
        ...
        <div className="time-greeting" style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
          ...
          <span style={{ fontSize: '0.85rem', color: '#64748b' }}>{greeting}</span>
        </div>
            ...
            >
              ...
              {notifications.length === 0 ? (
                ...
                notifications.map((item) => (
                  ...
                  >
                    <div style={{ fontWeight: 800, marginBottom: '6px' }}>{item.rule_violated}</div>
                    <div style={{ color: '#475569', fontSize: '0.85rem', marginBottom: '8px' }}>
                      {item.description}
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      ...
                      <span style={{ fontWeight: 700, color: item.severity === 'CRITICAL' ? '#ef4444' : '#f59e0b' }}>
                        {item.severity}
                      </span>
                    </div>
                  </div>
